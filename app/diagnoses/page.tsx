...
async function deleteItem(formData: FormData) {
    ...
    WHERE id = ${id}
  `.catch((error) => {
		console.error('Failed to delete diagnose: ', error)
	})

	revalidatePath('/diagnoses')
}

async function getItems(params: { diagnosis_name?: string }) {
	const { diagnosis_name } = params
	...
	return sql`
			SELECT * FROM diagnoses
			WHERE diagnosis_name = ${diagnosis_name};
	`
}

export default async function Page(props: { searchParams: { diagnosis_name?: string } }) {
	const { diagnosis_name } = props.searchParams
	const data = await getItems(props.searchParams)
	const { rows: diagnoses } = data

	return (
		<div className="relative overflow-x-auto">
			<h2 className="text-4xl font-extrabold mb-4">Диагнозы</h2>
				...
				<form className="rounded-xl border border-gray-200 bg-white p-6 shadow-lg">
					...
					<div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
						<div className="flex flex-col">
							<label htmlFor="diagnosis_name" className="text-stone-600 text-sm font-medium">Название диагноза</label>
							<input type="text" id="diagnosis_name" name="diagnosis_name" defaultValue={diagnosis_name}
