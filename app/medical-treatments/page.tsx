...
async function getItems(params: { name?: string }) {
	const { name } = params
...
export default async function Page(props: { searchParams: { name?: string } }) {
	...
	const { rows: medical_treatments } = data

	return (
		<div className="relative overflow-x-auto">
			<h2 className="text-4xl font-extrabold mb-4">Лечебные процедуры</h2>

			<Link href='/medical-treatments/new'>
				...
				<form className="rounded-xl border border-gray-200 bg-white p-6 shadow-lg">
					...
					<div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
						<div className="flex flex-col">
							<label htmlFor="name" className="text-stone-600 text-sm font-medium">Название</label>
				...
				{medical_treatments.map((item) => (
						<tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700" key={item.id}>
							...
							<td className="px-6 py-4 flex justify-left">
								<Link href={`/medical-treatments/${item.id}`}>
