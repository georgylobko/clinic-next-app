...
async function getItems(params: { medication_name?: string }) {
	const { medication_name } = params
...
export default async function Page(props: { searchParams: { medication_name?: string } }) {
	const { medication_name } = props.searchParams
	const data = await getItems(props.searchParams)
	const { rows: medications } = data

	return (
		<div className="relative overflow-x-auto">
			<h2 className="text-4xl font-extrabold mb-4">Препараты</h2>
				...
				<form className="rounded-xl border border-gray-200 bg-white p-6 shadow-lg">
					...
					<div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
						<div className="flex flex-col">
							<label htmlFor="medication_name" className="text-stone-600 text-sm font-medium">Название</label>
							<input type="text" id="medication_name" name="medication_name" defaultValue={medication_name}
