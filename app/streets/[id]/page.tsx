...
async function create(formData: FormData) {
    ...
    VALUES (${name})
  `.catch((error) => {
		console.error('Failed to create a street')
	})
...
export default async function Page({ params }: { params: { id: string } }) {
	...
	if (params.id !== 'new') {
		const result = await sql`SELECT * FROM streets WHERE id = ${params.id}`
		data = result.rows[0]
	}
	...
	return (
			<form className="max-w-sm mx-auto" action={params.id === 'new' ? create : update}>
				...
				<div className="mb-5">
					<label htmlFor="name" className="block mb-2 text-sm font-medium text-white-900">Название улицы</label>
